...
def test_comment_001(settings, parser):
    """commented import 1, ensure 'remove_comments' is applied"""
    result = parser.parse("""//@import "compass/css3";""")
    assert result == []


def test_comment_002(settings, parser):
    """commented import 2, ensure 'remove_comments' is applied"""
    result = parser.parse("""//      @import "compass/css3";""")
    assert result == []


def test_comment_003(settings, parser):
    """commented import 3, ensure 'remove_comments' is applied"""
    result = parser.parse("""/*
        @import "compass/css3";
        */""")
    assert result == []
